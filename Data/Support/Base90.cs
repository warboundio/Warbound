...
public static class Base90
{
    ...
    public static string Encode(int number) => Encode((ulong)number);

    public static List<string> Encode(List<int> numbers)
    {
        List<string> values = new(numbers.Count);
        foreach (int number in numbers) { values.Add(Encode(number)); }
    ...
    {
        if (number > MaxValue) { throw new ArgumentOutOfRangeException(nameof(number), $"Base90 only supports values up to {MaxValue}."); }

        Span<char> buffer = stackalloc char[EncodedLength];
        buffer.Fill(' ');

        int pos = EncodedLength;
